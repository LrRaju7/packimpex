...
export const getGlobalPageUrls = async (setGlobalPageURL) => {
  let pgUrls = [];
...
export const getCustomPageUrls = async (setPageURL,selectedLanguage,selectedCountry) => {
  let pgUrls = [];
  const result = await axios.get(DRUPAL_API_ENDPOINT +'/'+selectedCountry+ FETCH_ALL_URLS);
  let next = result.data.links?.next ? result.data.links.next : null;
  pgUrls.push(result.data.data);
  while (next) {
    const nextUrl = next.href;
...
export const getUserIPData = async (setUserLanguage, setUserCountry, setLoading, IP_API) => {
  const result = await axios.get(IP_API);
  setUserLanguage(result.data.countryCode);
...
export const getCardSliderData = async (
  ...
  if (imgApi) {
    ...
    if (imgName && imgFullDate) {
      setImgUrl(
        `${DRUPAL_API_ENDPOINT}/sites/default/files/${imgYear}-${imgMonth}/${imgName}`
      );
    } else {
      setImgUrl(
        DRUPAL_API_ENDPOINT + "/sites/default/files/2022-12/Basel-3703.jpg"
      );
    }
  }
  setLoading(false);
